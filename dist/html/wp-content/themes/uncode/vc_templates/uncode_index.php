...
$title = $index_type = $isotope_mode = $index_back_color = $items = $filtering = $filter_style = $filter_back_color = $filtering_full_width = $filtering_position = $filtering_uppercase = $filter_all_opposite = $filter_mobile = $filter_scroll = $footer_style = $footer_back_color = $footer_full_width = $pagination = $infinite = $infinite_hover_fx = $infinite_button = $infinite_button_text = $infinite_button_shape = $infinite_button_outline = $infinite_button_color = $style_preset = $images_size = $thumb_size = $single_width = $single_height = $single_height_viewport = $single_height_viewport_minus = $single_back_color = $single_shape = $radius = $single_text = $single_image_position = $single_vertical_text = $single_image_size = $single_lateral_responsive = $single_elements_click = $single_text_visible = $single_text_anim = $single_text_anim_type = $single_overlay_visible = $single_overlay_anim = $single_image_coloration = $single_image_color_anim = $single_image_anim = $single_reduced = $single_reduced_mobile = $single_padding = $single_text_reduced = $single_h_align = $single_h_align_mobile = $single_v_position = $single_h_position = $single_style = $single_overlay_color = $single_overlay_coloration = $single_overlay_opacity = $single_shadow = $shadow_weight = $shadow_darker = $single_border = $single_icon = $single_title_transform = $single_title_weight = $single_title_family = $single_title_dimension = $single_title_semantic = $single_title_height = $single_title_space = $single_text_lead = $single_css_animation = $single_animation_delay = $single_animation_speed = $carousel_height = $carousel_v_align = $carousel_type = $carousel_interval = $carousel_navspeed = $carousel_loop = $carousel_nav = $carousel_nav_mobile = $carousel_nav_skin = $carousel_dots = $carousel_dots_space = $carousel_dots_mobile = $carousel_dots_inside = $carousel_autoh = $carousel_lg = $carousel_md = $carousel_sm = $gutter_size = $inner_padding = $stage_padding = $carousel_overflow = $post_items = $portfolio_items = $page_items = $product_items = $screen_lg = $screen_md = $screen_sm = $filter = $el_id = $lbox_skin = $lbox_dir = $lbox_title = $lbox_caption = $lbox_social = $lbox_deep = $lbox_no_tmb = $lbox_no_arrows = $no_double_tap = $el_class = $orderby = $order = $custom_order = $order_ids = $css_class = $filter = $filter_background = $filter_sticky = $offset = $search_query = $using_plugin = $post_matrix = $matrix_amount = $matrix_items = $single_fluid_height = $carousel_height_viewport = $carousel_height_viewport_minus = $auto_query = '';
$post_types = array();
$wc_filtered = array();
...
$matrix_items = json_decode( base64_decode( strip_tags( $matrix_items ) ), true);
...
if ($index_type == 'isotope') {
  ...
  if ($carousel_height !== '' && $thumb_size !== 'fluid') $container_classes[] = 'owl-height-' . $carousel_height;
  if ($thumb_size === 'fluid') $container_classes[] = 'owl-height-viewport';
          ...
          <div class="isotope-filters menu-container <?php echo esc_attr($gutter_size) . esc_attr($filter_background); if ($filter_mobile === 'yes') echo ' mobile-hidden table-hidden'; if ($filter_scroll === 'yes') echo ' filter-scroll'; if ($inner_padding === 'yes') echo ' filters-inner-padding'; if ($filter_sticky === 'yes') echo ' sticky-element';?>">
              <div class="menu-horizontal<?php if ($filtering_full_width !== 'yes') echo ' limit-width'; ?> menu-<?php echo esc_attr($filter_style); ?> text-<?php echo esc_attr($filtering_position); ?>">
                  <ul class="menu-smart<?php if ($filtering_uppercase === 'yes') echo ' text-uppercase'; ?>">
                      ...
                      <?php else: ?>
                          <a href="<?php echo esc_url( $current_url ); ?>" class="<?php if (!isset($_GET['ucat'])) echo 'active'; ?>">
                      <?php endif; ?>
                          <?php esc_html_e( 'all', 'uncode' ) ?></a>
                          </span>
                      </li>
    ...
    switch ($index_type) {
        ...
        case 'carousel':
            ...
            if ($carousel_dots === 'yes' || $carousel_dots_mobile === 'yes') {
                if ($carousel_dots_space === 'yes') $container_classes[] = 'owl-dots-db-space';
        ...
        if ( $posts_counter > 0 ):
          ...
          foreach ( $posts as $post ):
			...
			if ( $post_matrix === 'matrix' ) {
				...
				$item_prop = (isset($matrix_items[($i_matrix % $matrix_amount) . '_i'])) ? $matrix_items[($i_matrix % $matrix_amount) . '_i'] : '';
			} else {
            ...
            if ($index_type !== 'carousel') {
              ...
              $single_width = floor(12 / $carousel_lg);
            }
            ...
            if ($single_text === 'overlay') {
                ...
                if ($single_reduced !== '') {
                    ...
                    if ($single_reduced_mobile !== '') $block_classes[] = 'tmb-overlay-text-wide-sm';
                }
            ...
            if ($single_title_semantic !== '') $block_data['tag'] = $single_title_semantic;
            ...
            if (isset($typeLayout['media']) && $item_thumb_id === '' && $single_text !== 'overlay') {
              if ($post->type === 'product' && $typeLayout['media'][0] === 'featured') {
                ...
                if ($single_back_color === '' && isset($item_prop) && is_array($item_prop)) $item_prop['single_padding'] = 0;
              }
            }
            ...
            if (isset($typeLayout['icon'][0]) && $typeLayout['icon'][0] !== '') $block_data['icon_size'] = ' t-icon-size-' . $typeLayout['icon'][0];
